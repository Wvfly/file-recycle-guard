...
class Database:
    ...
    def __init__(self, host: str, port: int, user: str, password: str,
        ...
        self._lock = threading.Lock()
        # P1-7: 连接池（扩大容量以支撑备份 worker + Web 并发）
        self._pool = ConnectionPool(max_size=64, config=self._config)
        # P1-6: 增量统计内存缓存
    ...
    def init_tables(self):
        ...
        with self._conn() as conn:
            ...
            """)
                ...
                """)
                # 注意：file_identity 和 fs_event 由 SQLite UsnEventStore 管理，不在 MySQL 中创建
                conn.commit()
    ...
    def upsert_backup_meta(self, watch_root: str, rel_path: str,
        ...
        with self._conn() as conn:
            ...
            # P1-6: 增量更新统计
            if old_row is None:
                self._adjust_backup_stats(1, file_size, conn=conn)
            else:
                old_size = old_row[0] or 0
                self._adjust_backup_stats(0, file_size - old_size, conn=conn)
    ...
    def batch_upsert_backup_meta(self, items: List[Dict]):
        ...
        with self._conn() as conn:
            ...
            if new_count != 0 or size_delta != 0:
                self._adjust_backup_stats(new_count, size_delta, conn=conn)
    ...
    def delete_backup_meta(self, watch_root: str, rel_path: str):
        ...
        with self._conn() as conn:
            ...
            if old_row is not None:
                self._adjust_backup_stats(-1, -(old_row[0] or 0), conn=conn)
    ...
    def _do_adjust_stats(self, conn, count_delta: int, size_delta: int,
                         count_key: str, size_key: str):
        """在给定连接上执行 stats 表原子更新"""
        with conn.cursor() as cur:
            if count_delta != 0:
                cur.execute(
                    "INSERT INTO stats (name, value) VALUES "
                    "(%s, %s) ON DUPLICATE KEY UPDATE value = value + %s",
                    (count_key, count_delta, count_delta)
                )
            if size_delta != 0:
                cur.execute(
                    "INSERT INTO stats (name, value) VALUES "
                    "(%s, %s) ON DUPLICATE KEY UPDATE value = value + %s",
                    (size_key, size_delta, size_delta)
                )
        conn.commit()

    def _adjust_backup_stats(self, count_delta: int, size_delta: int, conn=None):
        ...
        try:
            if conn is not None:
                self._do_adjust_stats(conn, count_delta, size_delta,
                                      'backup_count', 'backup_size')
            else:
                with self._conn() as c:
                    self._do_adjust_stats(c, count_delta, size_delta,
                                          'backup_count', 'backup_size')
        except Exception:
            with self._stats_lock:
            ...
            with self._conn() as conn:
                ...
                with self._stats_lock:
                    if "backup_count" in stats_map:
                        ...
                        self._recompute_stats_from_tables(conn)
    ...
    def _recompute_stats_from_tables(self, conn=None):
    ...
    def insert_recycle_meta(self, recycle_path: str, original_path: str,
        ...
        with self._conn() as conn:
            ...
            conn.commit()
            # P1-6: 增量更新回收站统计
            self._adjust_recycle_stats(1, file_size, conn=conn)
    ...
    def delete_recycle_meta(self, recycle_path: str):
        ...
        with self._conn() as conn:
            ...
            conn.commit()
            if old_row is not None:
                self._adjust_recycle_stats(-1, -(old_row[0] or 0), conn=conn)
    ...
    def delete_expired_recycle_meta(self, cutoff: float,
        ...
        with self._conn() as conn:
            ...
            while True:
                ...
                conn.commit()
                # P1-6: 增量更新回收站统计
                self._adjust_recycle_stats(-len(rows), -batch_size_sum, conn=conn)
    ...
    def _adjust_recycle_stats(self, count_delta: int, size_delta: int, conn=None):
