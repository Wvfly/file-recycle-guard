...
from .event_store import UsnEventStore
...
class UsnJournalMonitor:
    ...
    def set_exclude_rules(self, patterns: List[str], dirs: List[str]):
        """设置排除规则"""
        self._exclude_patterns = list(patterns) if patterns else []
        self._exclude_dirs = list(dirs) if dirs else []
...
def create_usn_watcher(config, logger):
    ...
    monitor.set_exclude_rules(
        getattr(config, 'exclude_patterns', []),
        getattr(config, 'exclude_dirs', []),
    )
