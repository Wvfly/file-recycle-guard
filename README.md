...
╚══════════════════════════╦═══════════════════════════╦═══════════════════════╝
              ...
              │  🅰 通道 A ─ 实时检测    │  │  🅱 通道 B ─ 主动扫描  │
              │                         │  │                       │
              │  📡 watchdog Observer   │  │  🔄 SyncThread        │
              │  ─────────────────────  │  │  ───────────────────  │
              │  on_created   → 备份    │  │  每 30s 全量遍历       │
              │  on_modified  → 备份    │  │  mtime+size 缓存比对   │
              │  on_deleted   → 延迟确认 │  │  变化文件 → backup()  │
              │  on_moved     → 备份+回收│  │  弥补 SMB 检测遗漏    │
              └────────────┬────────────┘  └───────────┬───────────┘
                           │                           │
                           └─────────┬─────────────────┘
                                     ▼
...
║  │ ─────────────────── │  │ ─────────────────────────────────────────  │  ║
║  │ ✦ 过期回收站清理    │  │ ✦ 线程安全连接池    ✦ 优雅降级策略        │  ║
║  │ ✦ 孤立备份回收      │  │ ✦ backup_meta 表    ✦ recycle_meta 表     │  ║
║  │ ✦ 宽限期保护        │  │ ✦ 文件方式回退兼容  ✦ UPSERT 原子操作     │  ║
║  ╰──────────────────────╯  ╰───────────────────────────────────────────╯  ║
║                                                                            ║
║  ╭──────────────────────────────────────────────────────────────────────╮  ║
║  │  🌐 Web 管理界面 ─── FastAPI + Uvicorn + Jinja2                     │  ║
║  │  ┌──────────────────────────────────────────────────────────────┐   │  ║
║  │  │  ✦ HTTP Basic Auth     ✦ 暗色主题 UI                        │   │  ║
║  │  │  ✦ 文件浏览 / 搜索     ✦ 一键恢复 / 清空                    │   │  ║
║  │  └──────────────────────────────────────────────────────────────┘   │  ║
║  ╰───────────────────────────────────────────────────────────────────────╯  ║
╚══════════════════╦═════════════════════════════╦════════════════════════════╝
...
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
  ...
  ┌──────────────┐
  │ 遍历目录树    │──→ mtime+size ──→ 缓存命中 ✅ ──→ ⏭️ 跳过（无变化）
  │ (每 30 秒)   │    缓存比对
  └──────────────┘      │
                   缓存未命中 ❌
                        │
                        ▼
                 ┌──────────────┐
                 │ backup_file()│──→ 📦 backup_dir + 🗄️ MySQL
                 └──────────────┘
...
║  │ 🧵 watchdog Observer    │   │ 🧵 SyncThread                        │   ║
║  │ ─────────────────────── │   │ ──────────────────────────────────── │   ║
║  │ ✦ 监听文件系统事件       │   │ ✦ 每 30s 全量遍历目录树              │   ║
║  │ ✦ 事件去重 (deque 500)  │   │ ✦ 内存缓存 mtime+size (Dict)         │   ║
║  │ ✦ created/modified      │   │ ✦ 缓存命中 → 跳过 (快速路径)         │   ║
║  │   → backup_file()       │   │ ✦ 缓存未命中 → backup_file()         │   ║
║  │ ✦ deleted → 延迟队列    │   │ ✦ 10万文件: 20万I/O → 10万 stat     │   ║
║  └─────────────────────────┘   └──────────────────────────────────────┘   ║
║                                                                            ║
║  ┌─────────────────────────┐   ┌──────────────────────────────────────┐   ║
...
║  🔒 线程安全机制                                                            ║
║  ┌──────────────────────────────────────────────────────────────────────┐   ║
║  │  🔐 _backup_lock      全局备份锁，防止并发文件操作冲突               │   ║
║  │  🔐 _pending_lock     删除队列锁，保护待确认删除列表                  │   ║
║  │  🔐 _db_lock          数据库初始化锁，保证单例                        │   ║
║  │  📋 _file_cache       sync 模块内存缓存，避免重复 I/O                │   ║
║  └──────────────────────────────────────────────────────────────────────┘   ║
╚══════════════════════════════════════════════════════════════════════════════╝
```
...
║  │  │ ──────────────────────── │  │ ────────────────────────────  │   │    ║
║  │  │ 🔑 rel_path    (UK)     │  │ 🔑 recycle_path    (UK)      │   │    ║
║  │  │ 🔑 watch_root  (UK)     │  │    original_path              │   │    ║
║  │  │    file_hash   (SHA256)  │  │    relative_path              │   │    ║
║  │  │    file_size   (BIGINT)  │  │    is_directory   (TINYINT)   │   │    ║
║  │  │    mtime       (DOUBLE)  │  │    deletion_time  (DOUBLE)    │   │    ║
║  │  │    source_path           │  │    file_size      (BIGINT)    │   │    ║
║  │  │    backup_time           │  │    file_hash      (VARCHAR)   │   │    ║
║  │  └──────────────────────────┘  └──────────────────────────────┘   │    ║
║  └────────────────────────────────────────────────────────────────────┘    ║
║                                ▲                                           ║
║                                │  优先读写                                  ║
...
| **双通道检测** | watchdog + 定期扫描 | watchdog 在 SMB 场景下不可靠，定期扫描弥补遗漏 |
| **哈希去重备份** | SHA256 比对 | 避免文件未变化时重复复制，减少 I/O 开销 |
| **内存缓存** | mtime+size 快速路径 | 10 万文件目录每轮扫描从 20 万次 I/O 降至 10 万次 stat |
